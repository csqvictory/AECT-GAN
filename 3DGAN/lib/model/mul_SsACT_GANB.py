...
from lib.model.loss import getLungCenterSeg
...
class CTGAN(Base_Model):
  ...
  def init_network(self, opt):
    ...
    self.D_loss = 0.0
    self.D_croploss = 0.0
    self.D_resizeloss = 0.0
    assert len(self.multi_view) > 0

    self.loss_names = ['D', 'G']
    self.metrics_names = ['Mse', 'CosineSimilarity', 'PSNR']
    self.visual_names = ['G_real', 'G_fake', 'G_input1', 'G_input2', 'G_Map_fake_F', 'G_Map_real_F', 'G_Map_fake_S', 'G_Map_real_S','G_Map_fake_V', 'G_Map_real_V']
  ...
  def set_input(self, input):
    self.input_xray1_sober = self.gradLayer(input[1][0])
    self.input_xray2_sober = self.gradLayer(input[1][1])
  ...
  def projection_visual(self):
    ...
    self.G_Map_real_V = self.transition(self.output_map(self.ct_unGaussian(self.G_real), 1))
    self.G_Map_fake_V = self.transition(self.output_map(self.ct_unGaussian(self.G_fake), 1))
    # map F is projected in dimension of H
  ...
  def forward(self):
    ...
    self.G_fake_D1, self.G_fake_D2, self.G_fake_D = self.netG([self.G_input1, self.G_input2, self.input_xray1_sober, self.input_xray2_sober])
  ...
  def backward_D_basic(self, D_network, input_real, input_fake, fake_pool, criterionGAN, loss_weight):
    ...
    dPecpt_lambda = self.opt.dPecpt_D_lambda
    ...
    gan_D_volumeMse = self.getVolumeLoss(D_real_pred[0], D_real_pred[1],idt_lambda)
    gan_D_plips_loss = get_discirmator_realimage_loss(D_real_pred[0], D_real_pred[1])*dPecpt_lambda
    gan_loss = (gan_loss_fake + gan_loss_real) * loss_weight + gan_D_plips_loss + gan_D_volumeMse
    gan_loss.backward()
    return gan_loss, gan_loss-gan_D_plips_loss-gan_D_volumeMse, gan_D_plips_loss, gan_D_volumeMse
...
# 添加辨别器的condition
  ...
  def backward_G(self):
    ...
    map_m_lambda = self.opt.map_projection_lambda
    gPecpt_lambda = self.opt.gPecpt_G_lambda
    ...
    if idt_lambda > 0:
      ...
      if self.opt.idt_reduction == 'none' and self.opt.idt_weight > 0:
        ...
        self.G_lpips_loss = get_generator_featuremap_loss(self.G_fake_D, self.G_real_D) * gPecpt_lambda
    part = random.randint(0, 7)
    D_real_pred = self.netD(real_input,True,  part = part)

    # feature metric loss
    if fea_m_lambda > 0:
      # self.loss_fea_m = self.feature_metric_loss(D_fake_pred[-1], D_real_pred[-1], loss_weight=fea_m_lambda, num_D=self.opt.num_D, feat_weights=4.0 / (self.opt.n_layers_D + 1), criterionFea=self.criterionFea)
      self.loss_fea_m = F.mse_loss(D_real_pred[-1], D_fake_pred[-1])
    ...
    self.loss_total_G = self.loss_G + 0.0 + self.G_lpips_loss 
    if idt_lambda > 0:
  ...
  def optimize_parameters_G(self,epoch_i):
    # forward
    self()
  ...
  def optimize_parameters_D(self,epoch_i):
    self()
  ...
  def return_D_epochLoss(self):
    return  self.loss_D

  def return_G_epochLoss(self):
    return self.loss_total_G

  def return_D_G_epochLoss(self):
    return self.loss_total_G, self.G_lpips_loss, self.loss_D, self.D_loss, self.gan_D_plips_loss, self.gan_D_volume_mse
